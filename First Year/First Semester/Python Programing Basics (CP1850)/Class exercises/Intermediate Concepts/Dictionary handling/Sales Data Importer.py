...
def get_date():
   ...
   while True:
        ...
        if day >0  and month == 2:
            ...
            return month, quarter, day
            break
        elif (day > 0 and day <=30 and day <=31) and (month ==1 or month ==3 or month ==5 or month ==7 or month == 8 or month ==10 or month ==12):
            return month, quarter, day
            break
        else:
            print('Please enter a valid data')
...
def view(SALES):
    if len(SALES)==0:
        ...
        i = 1
        for sale in SALES:
            print('{}   {}-{}-{}\t\t\t\t{}\t\t\t{}{}${:,.2f}'.format(i,sale['year'],sale['month'],sale['day'],sale['quarter'],reg[sale['region']]," "*(14-len(reg[sale['region']])),float(sale['amount'])))
            i +=1
        print('='*65)
        total = []
        for sale in SALES:
