...
BEGIN
    ...
    IF pet_id_param is null or pet_name_param is null or species_param is null or sex_param is null or sex_param is null or dob_param is null or chipped_param is null then
		SIGNAL SQLSTATE 'PET INFO ERROR'
			SET message_text = 'Must Enter Pet Info',
				MYSQL_ERRNO = 1407;
	ELSEIF client_Fname_param is null or client_Lname_param is null or phone_param is null then
		SIGNAL SQLSTATE 'CLIENT INFO ERROR'
				SET message_text = 'Must Enter CLIENT INFO',
					MYSQL_ERRNO = 2235;
	ELSEIF not sex_param = 'm' or not sex_param = 'f' then
		SIGNAL SQLSTATE 'PET SEX ERROR'
...
CALL ClientPetIns (100, 'Jack', 'German Shepherd', 'm', '2005-4-19', 2, 'Michael', 'Agonsi','7097645352','0','0','0','0');
